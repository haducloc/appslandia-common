...
public class ConverterProvider extends InitializingObject {
  ...
  protected void init() throws Exception {
    ...
    converters.putIfAbsent(Converter.STRING_UPPER, new ULCStringConverter(true));
    converters.putIfAbsent(Converter.STRING_LOWER, new ULCStringConverter(false));
    ...
    converters.putIfAbsent(Converter.INSTANT, new InstantConverter());
  ...
  public static void main(String[] args) {
    var provider = new ConverterProvider();
    provider.initialize();

    var converterClasses = provider.converters.values().stream().map(Converter::getClass)
        .map(c -> (Class<? extends Converter>) c).collect(Collectors.toCollection(LinkedHashSet::new));

    for (var converterClass : converterClasses) {
      try {
        var errorMsgKeyField = ReflectionUtils.findField(converterClass, "ERROR_MSG_KEY");
        var errorMsgKey = errorMsgKeyField.get(null);
